...
public interface EventService extends CrudRepository<Event,Integer> {
    ...
    @Query("select e from Event e where e.userclazz.iduser = :iduser")
    List<Event> findByUser(@Param("iduser") int iduser);

    @Query("select e.id from Event e where e.userclazz.iduser = :iduser and e.allevent.idevent = :idevent")
    List<Integer> findIdByUserAndEvent(@Param("iduser") int iduser, @Param("idevent") int idevent);

    @Query("select e.id from Event e where e.userclazz.iduser = :iduser")
